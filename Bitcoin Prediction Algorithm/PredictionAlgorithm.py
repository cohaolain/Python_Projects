...
initDate = datetime.datetime.fromtimestamp(initialValues[0]).strftime('%Y-%m-%d')
...
for i in range(numDaysPredict):
    date = datetime.datetime.fromtimestamp(initialValues[0]+((i+1)*86400)).strftime('%Y-%m-%d')

    # answer = initialValues[1]*averageGrowth**(i+1)
